...
namespace SPFactory.Core.Factory
{
    /// <summary>
    /// 
    /// </summary>
    public class WorkbookManager
    {
        ...
        public void MountSpreadsheet(Spreadsheet spreadsheetFactory)
        {
            if (spreadsheetFactory.SpreadsheetList != null)
            {
                foreach (var item in spreadsheetFactory.SpreadsheetList)
                {
        ...
        private void CreateSpreadsheet(Spreadsheet spreadsheetFactory)
        {
            if (spreadsheetFactory != null)
            {
                ...
                if (spreadsheetFactory.Datasource != null)
                {
                    ...
                    if (!String.IsNullOrEmpty(spreadsheetFactory.MergedTitle))
                    {
                        ...
                        //TODO: CreateContentTableCells(sheet, spreadsheetFactory, firstRow);
                        firstRow++;// = sheet.LastRowNum;
                    }
                    else
                    {
                    ...
                    {
                        //TODO: corrigir
                        //TODO: SetContentTableCellsValue(sheet, firstRow - 1, 0, spreadsheetFactory.Properties, spreadsheetFactory.Datasource, spreadsheetFactory.ChildSheet, spreadsheetFactory.MergedTitle);
                    }
                    else
                    {
                        //TODO: SetContentTableCellsValue(sheet, firstRow, 0, spreadsheetFactory.Properties, spreadsheetFactory.Datasource, spreadsheetFactory.ChildSheet, spreadsheetFactory.MergedTitle);
        ...
        private List<PropertyCell> GetPropertiesCells(Spreadsheet spreadsheetFactory)
        {
        ...
        //TODO: corrigir implementação para que suporte multimpas linhas com filhos
        ...
        private void PrepareTableHeader(HSSFSheet sheet, Spreadsheet spreadSheet, int cell)
        {
        ...
        private void CreateContentTableCells(HSSFSheet sheet, Spreadsheet spreadsheet, int firstRow)
        {
            ...
            if (!String.IsNullOrEmpty(spreadsheet.MergedTitle))
            {
                ...
                if (spreadsheet.SpanTitleStyle != null)
                {
                    for (int j = spreadsheet.FirstCell; j < spreadsheet.FirstCell + spreadsheet.Properties.Length; j++)
                    {
                        //adicionar formatação em linhas com ceulas agrupadas
